...
int do_exec(int nargs, char **args)
{
    ...
    par[i] = (char*)0;
    pid = fork();
    INFO("Getting for pid '%d' after fork() system call when exec the command '%s'.", pid, par[0]);
    if (pid<0)
    {
       ERROR("Error while forking a process when executing a command '%s'.", par[0]);
    }
    ...
    if (pid == 0)
    {
        ...
        INFO("Ready to go to exec the command '%s'.", par[0]);
        execve(par[0], par, environ);
        exit(0);
    }
    else if (pid > 0)
    {
        INFO("Waiting for finish the executed command '%s'.", par[0]);
        while(wait(&status)!=pid);
        INFO("Command '%s' finished.", par[0]);
    }
    return 0;
}

...
#ifdef WIFI_DRIVER_LOADER_DELAY
	/* Archos devices are a little bit slow loading wlcore firmware (from kernel backports)... we need that hack at this point to don't let the following checks fail. */
        ALOGE("Sleeping a little bit <%d>\n",WIFI_DRIVER_LOADER_DELAY);
        usleep(WIFI_DRIVER_LOADER_DELAY);
#endif
